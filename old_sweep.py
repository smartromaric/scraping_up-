...
def scan_fleet_rows(driver, max_rows: int | None = None) -> list[tuple[int, object]]:
    ...
    for attempt in range(1, 4):
        try:
            ...
            return _parse_table_rows(rows, limit)
        except Exception:
...
def find_row_for_candidate(
    ...
    *,
        ...
        try:
            if qa.open_filters(driver, 0) and qa.set_filter_matricule(driver, matricule, 0):
                if qa.apply_filters(driver, 0, wait_matricule=matricule):
                    ...
                    picked = _pick_row_among_matches(
    ...
    while time.time() < end:
        matching = [
            (i, r)
            for i, r in scan_fleet_rows(driver, max_rows=_SCAN_TOP)
        ]
...
def collect_candidates(
    ...
    *,
    ...
    """
    Cibles = flotte EN ATTENTE (haut du tableau) ; statut doc sur la page document.
    Par défaut ne lit que les scan_top premières lignes (récentes en haut).
    """
    top = _SCAN_TOP if scan_top is None else scan_top
    limit_label = "tout le tableau" if top <= 0 else f"top {top}"

    candidates = []
    seen_keys: set[str] = set()
    rows_info: list[dict] = []
    scanned = scan_fleet_rows(driver, max_rows=top)
    n_pending = 0
    prev_matricule = ""
    ...
    if not debug:
        log(
            ...
            f"EN ATTENTE: {n_pending} | candidats: {len(candidates)}"
        )
        for c in candidates:
            log(
    ...
    if visual:
        ...
        _ui_sweep_banner(
            driver,
            f"{len(candidates)} candidat(s) — {limit_label} — "
            "rouge=candidat, orange=EN ATTENTE, vert=OK",
            "#1565c0",
        )
...
def navigate_manage_fleet_full(driver) -> bool:
    """Ouvre manage-fleet sans filtre matricule, pagination 500."""
...
def find_row_by_key_all(
    ...
    while time.time() < end:
        for _, row in scan_fleet_rows(driver, max_rows=top):
            try:
                if qa._row_identity_key(row, matricule) == row_key:
                    return row
            except Exception:
...
def run_sweep(
    ...
    *,
    ...
    log(
        f"   Mode sweep : scan {scan_label} (récent en haut) → EN ATTENTE → "
        "page doc → approbation | pas d'upload"
    )
    if show_ui:
...
Exemples:
  ...
  python sweep_approve_pending_documents_vps.py --max-rounds 500
    ...
    rounds_desc = (
        f"max {args.max_rounds} tour(s)"
        if args.max_rounds is not None
        else "tours illimités (passer --max-rounds N pour plafonner)"
    )
    log(f"   Max {args.max_per_round} ligne(s)/tour | {rounds_desc}")
    log(
        f"   Scan : {'tout le tableau' if _SCAN_TOP <= 0 else f'top {_SCAN_TOP} lignes'} "
        f"(logs détaillés : --debug-scan)"
    )
    if qa._FAST_MODE:
    ...
    slack_rounds = (
        f"• Max {args.max_rounds} tours\n"
        if args.max_rounds is not None
        else "• Tours illimités (--max-rounds pour plafonner)\n"
    )
    qa.send_slack_message(
        "🔄 *Sweep approbation documents démarré*\n"
        f"• Max {args.max_per_round}/tour\n"
        + slack_rounds
