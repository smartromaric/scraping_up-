...
3) Remplit numéro + montant pour chaque ligne (CSV/XLSX ou state.json).
4) Te laisse choisir le destinataire + cliquer Transférer + valider popup.
5) Passe à la ligne suivante quand tu confirmes (o/n) dans le terminal.

Source state.json (critère : admin.reason == assigned_approved) :
  set PARTNER_INDEX=1
  python open_upjunoo_bulk_assisted_ivanup.py

Source CSV classique :
  set TRANSFER_FILE=output\\transfer_list.csv
  python open_upjunoo_bulk_assisted_ivanup.py
"""
...
PARTNER_EMAIL = os.getenv("PARTNER_EMAIL", "jesusvie99@gmail.com")
PARTNER_PASSWORD = os.getenv("PARTNER_PASSWORD", "123456789@")

# Fichier source des transferts (2 colonnes minimum: numero, montant)
TRANSFER_FILE = os.getenv("TRANSFER_FILE", "output/transfer_list_jesus_vie.csv")
PARTNER_INDEX = os.getenv("PARTNER_INDEX", "").strip()
PARTNER_STATE_FILE = os.getenv(
    "PARTNER_STATE_FILE",
    str(Path(__file__).resolve().parent / "output" / "partner_automation" / "state.json"),
)
TRANSFER_AMOUNT = os.getenv("TRANSFER_AMOUNT", "2000")
DEFAULT_PARTNER_PASSWORD = os.getenv("PARTNER_PASSWORD", "Upjunoo@Admin")
...
def go_to_transfer_modal(driver, wait: WebDriverWait):
    # Navigation ultra-rapide : boucle directe sans overhead WebDriverWait
    for label in ["Comptes", "Portefeuille", "Transférer"]:
        for attempt in range(10):  # max 5 secondes par bouton
            try:
...
def select_transfer_type_conducteur(driver, wait: WebDriverWait):
    """Clique sur le menu déroulant et choisit Conducteur via coordonnées."""
    try:
        # Masquer le clavier
        ...
        # On localise les EditText (montant et numéro) qu'on sait trouver
        edits = _visible_edittexts(driver)
        if len(edits) >= 1:
            # Le dropdown est JUSTE AU-DESSUS du premier champ (montant)
            first_edit = edits[0]
            loc = first_edit.location  # {'x': ..., 'y': ...}
            size = first_edit.size     # {'width': ..., 'height': ...}
            
            # On tape au centre horizontal, environ 80px au-dessus du champ montant
            tap_x = loc['x'] + size['width'] // 2
            tap_y = loc['y'] - 80
            
            print(f"👆 Tap sur dropdown à ({tap_x}, {tap_y})")
            driver.tap([(tap_x, tap_y)], 100)
            time.sleep(0.8)
            
            # Maintenant la liste est ouverte, on cherche 'Conducteur'
            # Le deuxième élément de la liste est en-dessous du premier
            # On essaie d'abord par accessibility_id ou texte
            found = False
            for method in [
                (AppiumBy.ACCESSIBILITY_ID, "Conducteur"),
                (By.XPATH, "//*[@text='Conducteur']"),
                (By.XPATH, "//*[contains(@text, 'Conducteur')]"),
            ]:
                try:
                    el = WebDriverWait(driver, 2, poll_frequency=0.1).until(
                        EC.element_to_be_clickable(method)
                    )
                    el.click()
                    found = True
                    print("✅ Type 'Conducteur' sélectionné")
                    break
                except:
                    continue
            
            if not found:
                # Dernier recours : tap sur la 2e option (environ 50px en dessous)
                driver.tap([(tap_x, tap_y + 50)], 100)
                print("✅ Type 'Conducteur' sélectionné (par coordonnées)")
            
            time.sleep(0.3)
        else:
            print("⚠️ Aucun EditText trouvé pour repérer le dropdown")
    except Exception as e:
        print(f"⚠️ Impossible de sélectionner 'Conducteur' : {e}")
...
def wait_user_then_prepare_next(
    driver,
    wait: WebDriverWait,
    row: Dict[str, str],
    *,
    ...
    # Cliquer sur le bouton Transférer de la MODAL par coordonnées
    try:
        try: driver.hide_keyboard()
        except: pass
        time.sleep(0.2)
        ...
        if len(edits) >= 2:
            last_edit = edits[-1]
            loc = last_edit.location
            size = last_edit.size
            tap_x = loc['x'] + size['width'] * 3 // 4
            tap_y = loc['y'] + size['height'] + 80
            driver.tap([(tap_x, tap_y)], 100)
            print(f"👆 Clic sur 'Transférer' ({tap_x}, {tap_y})")
        else:
            print("⚠️ Bouton Transférer non trouvé")
    except Exception as e:
        print(f"⚠️ Bouton Transférer non cliqué : {e}")
    
    time.sleep(0.5)

    # Confirmation
    label = row.get("name") or row["numero"]
    ok = input(
        f"✅ Transfert envoyé pour {label} ({row['numero']}). Est-ce bien passé ? (o/n): ",
    ).strip().lower() in {"o", "ok", "oui", "y", "yes"}
    if ok and use_state:
        from partner_state_mobile import mark_transfer_done

        mark_transfer_done(state_path, partner_index, row["numero"])
        print(f"📝 state.json mis à jour: {row['numero']} -> transfer_2000_done")
    elif ok and TRANSFER_FILE.lower().endswith(".csv"):
        mark_status_csv(TRANSFER_FILE, int(row["row_index"]), "OK")
        print(f"📝 Statut mis à jour: {row['numero']} -> OK")

    # Ré-ouvrir modal rapidement + attendre que les EditText soient prêts
    for attempt in range(30):  # max 3 secondes
        try:
            driver.find_element(AppiumBy.ACCESSIBILITY_ID, "Transférer").click()
            break
        except:
            time.sleep(0.1)
    
    # Attendre que la modal soit prête (EditText visibles)
    for attempt in range(30):  # max 3 secondes
        edits = _visible_edittexts(driver)
        if len(edits) >= 2:
            break
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Transferts assistés UpJunoo Driver")
    p.add_argument(
        "--partner-index",
        type=int,
        default=int(PARTNER_INDEX) if PARTNER_INDEX.isdigit() else None,
        help="Index partenaire (lit output/partner_automation/state.json)",
    )
    p.add_argument(
        "--state-file",
        default=PARTNER_STATE_FILE,
        help="Chemin vers state.json",
    )
    p.add_argument(
        "--amount",
        default=TRANSFER_AMOUNT,
        help="Montant par transfert (défaut 2000)",
    )
    p.add_argument(
        "--transfer-file",
        default=TRANSFER_FILE,
        help="CSV/XLSX si --partner-index non utilisé",
    )
    return p.parse_args()


def load_pending_rows(args: argparse.Namespace) -> tuple[List[Dict[str, str]], bool, int, str, str, str]:
    use_state = args.partner_index is not None
    if use_state:
        from partner_state_mobile import (
            TRANSFER_REASON_ELIGIBLE,
            build_transfer_queue,
            load_state,
            resolve_partner_credentials,
        )

        state = load_state(args.state_file)
        rows, partner, skipped = build_transfer_queue(
            state,
            args.partner_index,
            amount=args.amount,
            reason=TRANSFER_REASON_ELIGIBLE,
        )
        email, password = resolve_partner_credentials(
            partner,
            email_env=PARTNER_EMAIL,
            password_env=PARTNER_PASSWORD,
            default_password=DEFAULT_PARTNER_PASSWORD,
        )
        print(f"📄 state.json partenaire {args.partner_index} ({partner.get('email', '')})")
        print(f"   Critère: admin.reason == {TRANSFER_REASON_ELIGIBLE!r}")
        print(f"   Éligibles: {len(rows)} | ignorés: {len(skipped)}")
        for s in skipped:
            print(f"   ⏭ {s.get('name', '?')} | {s.get('phone', '')} | {s.get('skip', '')}")
        pending_rows = rows
        return pending_rows, True, args.partner_index, args.state_file, email, password

    ensure_csv_status_column(args.transfer_file)
    rows = load_rows(args.transfer_file)
    pending_rows = [r for r in rows if str(r.get("statut", "")).strip().upper() != "OK"]
    print(f"📄 {len(rows)} ligne(s) depuis {args.transfer_file} | à traiter: {len(pending_rows)}")
    return pending_rows, False, 0, "", PARTNER_EMAIL, PARTNER_PASSWORD


def main():
    args = parse_args()
    pending_rows, use_state, partner_index, state_path, email, password = load_pending_rows(args)
    if not pending_rows:
        print("✅ Aucun transfert en attente.")
        return
    ...
    try:
        print(f"🚀 Démarrage app + login ({email})...")
        open_app(driver)
        login_partner(driver, wait, email, password)
        go_to_transfer_modal(driver, wait)

        for i, row in enumerate(pending_rows, 1):
            phone = row["numero"]
            amount = row["montant"]
            who = row.get("name") or phone
            extra = ""
            if row.get("plate"):
                extra = f" | plaque {row['plate']}"
            print(f"\n[{i}/{len(pending_rows)}] {who} -> {phone} / {amount}{extra}")
            fill_transfer_fields(driver, phone, amount)
            select_transfer_type_conducteur(driver, wait)
            wait_user_then_prepare_next(
                driver,
                wait,
                row,
                use_state=use_state,
