...
def sync_fleet(driver, json_drivers: list, online_fleet: list, dry_run: bool = False) -> dict:
    ...
    - Suppression : en ligne mais absent du JSON (comparaison par matricule)
    - Création :
        1) matricule absent du site -> créer
        2) matricule déjà présent mais chauffeur différent -> créer l'association manquante
    """
    stats = {
    ...
    log(f"      • Doublons dans JSON  : {sum(1 for v in json_by_plate.values() if len(v) > 1)} clé(s)")
...
def main():
    ...
    log(f"\n{'='*70}")
    log("🔄 UPDATE FLEET VPS DRIVER ASSOC — Dédup + Ajout manquants")
    log(f"{'='*70}")
    if args.dry_run:
