...
BASE_DIR           = _SCRIPT_DIR.parent if _SCRIPT_DIR.name == "vps_deploy" else _SCRIPT_DIR
...
def count_vehicles(driver) -> int:
    try:
        ...
        WebDriverWait(driver, 20).until(
            EC.presence_of_element_located((By.CSS_SELECTOR, "table,#no-data,.empty"))
        )
        time.sleep(1)
...
def main():
    ...
    for p in raw:
        ...
        if not m:
            continue
        num = int(m.group(1))
        email = f"partenaire{num}@upjunoo.com"
    ...
    # Sauvegarde JSON
    out = BASE_DIR / "output" / "reports" / "fleet_count_report.json"
