...
def load_partner_email_map(data_root: Path) -> dict[str, str]:
    """
    Construit une map key->email à partir des dossiers partenaires.
    Ajoute les alias partenaireN et partenairesN pour le même numéro.
    """
    ...
    for folder in sorted([p for p in data_root.iterdir() if p.is_dir()]):
        ...
        num = extract_partner_num(folder_name)
        if num <= 0:
            continue
...
def read_excel_targets(excel_path: Path) -> dict[str, set[str]]:
    ...
    for row_idx in range(1, min(ws.max_row, 30) + 1):
        ...
        if "partenaire" in values and ("numero_apres" in values or "numero" in values):
            header_row = row_idx
            col_partner = values.index("partenaire")
            # Priorité au numéro enrichi depuis data.json
            col_phone = values.index("numero_apres") if "numero_apres" in values else values.index("numero")
            break

    if header_row is None or col_partner is None or col_phone is None:
        raise RuntimeError("Colonnes 'PARTENAIRE' et 'numero_apres/numero' introuvables dans le fichier Excel.")

    targets = defaultdict(set)
    for row in ws.iter_rows(min_row=header_row + 1, values_only=True):
        partner_raw = row[col_partner] if col_partner < len(row) else None
        phone_raw = row[col_phone] if col_phone < len(row) else None
        partner = normalize_partner(str(partner_raw or ""))
        phone = normalize_phone(phone_raw)
        if not partner or not phone:
            continue
        targets[partner].add(phone)
    return dict(targets)
...
def wait_for_table_data(driver, timeout: int = 35) -> bool:
    start = time.time()
...
def load_drivers_page(driver) -> None:
    ...
    if wait_for_table_data(driver, timeout=35):
        log("✅ Liste chauffeurs chargée après pagination 500")
    else:
        log("⚠️ Chargement liste chauffeurs lent/incomplet après pagination 500", "WARNING")


def scrape_driver_rows(driver) -> list[dict]:
    rows = driver.find_elements(By.CSS_SELECTOR, "table tbody tr")
    data = []
    for idx, row in enumerate(rows):
        try:
            ...
            # Evite les dépendances d'index de colonne: on cherche un numéro
            # dans les cellules de la ligne et on prend le plus pertinent.
            candidates = []
            for cell in tds:
                txt = (cell.text or "").strip()
                norm = normalize_phone(txt)
                if norm and len(norm) >= 8:
                    candidates.append((txt, norm))

            if not candidates:
                continue

            # Préférence au format long (souvent +225XXXXXXXXXX).
            phone, norm_phone = max(candidates, key=lambda x: len(x[1]))
            if not norm_phone:
                continue
            data.append({"idx": idx, "phone": phone, "norm_phone": norm_phone})
        except Exception:
            continue
    return data
...
def delete_targets_for_partner(driver, partner: str, phones: set[str], dry_run: bool = False) -> dict:
    stats = {"target_count": len(phones), "deleted": 0, "failed": 0, "not_found": 0}
    remaining = set(phones)
    max_passes = 30

    for pass_num in range(1, max_passes + 1):
        load_drivers_page(driver)
        rows = scrape_driver_rows(driver)
        if not rows:
            log(f"⚠️ Aucun chauffeur affiché pour {partner}", "WARNING")

        matches = [r for r in rows if r["norm_phone"] in remaining]
        if not matches:
            break

        matches.sort(key=lambda x: x["idx"], reverse=True)
        log(f"🗑️ {partner} | Pass {pass_num}: {len(matches)} chauffeur(s) à supprimer")

        for item in matches:
            phone = item["norm_phone"]
            if dry_run:
                log(f"   [DRY-RUN] Suppression simulée: {item['phone']}")
                stats["deleted"] += 1
                remaining.discard(phone)
                continue
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Supprime les chauffeurs listés dans un Excel, partenaire par partenaire.")
    parser.add_argument(
        "--excel",
    ...
    if not targets_by_partner:
        log("Aucun chauffeur à traiter selon les filtres.", "WARNING")
        return

    partner_email_map = load_partner_email_map(data_root)

    total_targets = sum(len(v) for v in targets_by_partner.values())
    log(f"📦 Cibles: {len(targets_by_partner)} partenaire(s), {total_targets} numéro(s) uniques")
    if args.dry_run:
    ...
    try:
        for partner in sorted(targets_by_partner.keys(), key=lambda x: int(re.findall(r"\d+", x)[0])):
            phones = targets_by_partner[partner]
            email = partner_email_map.get(partner) or derive_email(partner)
            log(f"\n{'='*65}")
            log(f"👤 Partenaire: {partner} | 📧 {email} | 🎯 {len(phones)}")
            log(f"{'='*65}")

            if not email:
                log(f"❌ Email impossible à déduire pour {partner}", "ERROR")
                summary[partner] = {"target_count": len(phones), "deleted": 0, "failed": 0, "not_found": len(phones)}
                continue
            ...
            stats = delete_targets_for_partner(driver, partner, phones, dry_run=args.dry_run)
