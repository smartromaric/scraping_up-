...
def setup_driver_headless(headed: bool = False):
    ...
    # Chercher d'abord ChromeDriver système (inclut /usr/local/bin)
    chromedriver_path = None
    for cd in ["/usr/local/bin/chromedriver", "/usr/bin/chromedriver", "/snap/bin/chromium.chromedriver", "chromedriver"]:
        if os.path.isfile(cd) or shutil.which(cd):
            chromedriver_path = cd if os.path.isfile(cd) else shutil.which(cd)
            break
    
    if chromedriver_path:
        log(f"✅ ChromeDriver système trouvé : {chromedriver_path}")
        service = Service(chromedriver_path)
    else:
        # Fallback sur ChromeDriverManager
        try:
            log("⏳ ChromeDriver système non trouvé, installation via ChromeDriverManager...")
            chromedriver_path = ChromeDriverManager().install()
            service = Service(chromedriver_path)
            log(f"✅ ChromeDriver installé : {chromedriver_path}")
        except Exception as e:
            raise RuntimeError(f"ChromeDriver non trouvé et installation échouée : {e}")
...
def main():
    ...
    parser.add_argument("--end", help="S'arrêter après ce partenaire (ex: Partenaire30)")
    ...
    # Filtrer si --end
    if args.end:
        end_num = extract_partner_number(args.end)
        partners_to_process = [p for p in partners_to_process if extract_partner_number(p.name) <= end_num]
        log(f"🏁 Arrêt après le partenaire <= {args.end}")
