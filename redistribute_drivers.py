...
def assign_driver(driver, driver_entry: dict, target_owner_id: str,
    ...
    """
    Ouvre la page d'édition du conducteur, extrait les données du formulaire,
    puis POST directement à /fleet-drivers/update/{id} via HTTP.
    ...
    try:
        ...
        form_data = driver.execute_script("""
            ...
            form.querySelectorAll('textarea').forEach(function(t) {
                var key = t.name || t.id;
                if (key) data[key] = t.value;
            });
            return data;
        """)

        if not form_data:
            raise Exception("Impossible d'extraire les données du formulaire")

        # Changer le partenaire
        form_data['owner'] = target_owner_id
        print(f"[owner→{target_owner_id[:8]}..] ", end="", flush=True)

        # Récupérer les cookies Selenium
        cookies = {}
        for c in driver.get_cookies():
            cookies[c['name']] = c['value']

        # Token XSRF (URL-décodé)
        xsrf_token = unquote(cookies.get('XSRF-TOKEN', ''))

        # Créer session HTTP avec les cookies
        session = http_requests.Session()
        for name, value in cookies.items():
            session.cookies.set(name, value)

        # POST vers l'endpoint update
        update_url = f"{BASE_URL}/fleet-drivers/update/{driver_id}"
        headers = {
        ...
        response = session.post(update_url, data=form_data, headers=headers, allow_redirects=True)
